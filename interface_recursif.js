...
var data1Pierre;
var copyMasquedata1Pierre;
var TableauGermesFaits = [];
var TableauPierreFaite = [];
...
function modifierMasque2(canvas, chemin, masque, centres) {
    ...
    for (let i = 0; i < centres.length; i++) {
        ...
        if (distancechemin(point, chemin) < distancemax) {
            germes.push(point);
            TableauGermesFaits.push([]);
            TableauPierreFaite.push(0);
        }
    }
    console.log(centres.length + ' centres au total')
    console.log(germes.length + ' germes obtenus')

    data1Pierre = canvasdata;
    copyMasquedata1Pierre = masquedata;
    var x;
    var y;
    for (let k=0;k<700;k++) {
        x = germes[k][0];
        ...
        y = germes[k][1];
        //console.log(y)
        recup1Pierre([x, y], w, h, []);
        //TableauGermesFaits[k] = [];
    }

    /*
    for (let k=0;k<20;k++) {
        while (TableauPierreFaite[k] != 0) {

        }
        TableauGermesFaits[k] = [];
    }*/
    //console.log(TableauGermesFaits)
    
    //console.log(TableauPierreFaite)
    //ctx.putImageData(canvas1Pierre, 0, 0);
    //console.log(TableauGermesFaits[0])
    /*
    for (let i = 0; i<data1Pierre.data.length; i++) {
        if (data1Pierre.data[i] != 0) {
            //data1Pierre.data[i] = 255;
            //console.log(data1Pierre.data[i]);
        }
    }*/
    console.log(data1Pierre.data)
    return data1Pierre
...
function estpresent(germes, i, j) {
    k = 0
    while (k < germes.length) {
        germe = germes[k];
        if (i == germe[0] && j == germe[1]) {
            return true;
        }
        k = k + 1;
    }
...
function egaliteTab(germes, copygermes) {
    if (germes[germes.length - 1] != copygermes[copygermes.length - 1]) {
        return false;
    } else {
        return true;
    }
...
function recup1Pierre(germe, w, h, tab) {

    //TableauPierreFaite[k]++;
    var x = germe[0];
    var y = germe[1];
    ...
    for (let i=-1; i<=1; i++) {
        for (let j=-1; j<=1; j++) {
                ...
                if (new_x >= 0 && new_y >= 0) {
                    var n =  new_x * w + new_y;
                    //var tabgermes  = TableauGermesFaits[k];
                    if (! tab.includes(n)) {
                        if (data1Pierre.data[4*n] == 0 && copyMasquedata1Pierre.data[4*n] == 255) {
                            data1Pierre.data[4*n] = 255;
                            data1Pierre.data[4*n + 1] = 255;
                            data1Pierre.data[4*n + 2] = 255;
                            data1Pierre.data[4*n + 3] = 255;
                            tab.push(n);
                            if (x != new_x && y != new_y) {
                                recup1Pierre([new_x,new_y], w, h, tab);
                            }
...
function propagation(germes, masquedata, canvasdata, w, h) {
    var copygermes = germes;
    for (let k = 0; k < germes.length; k++) {
        ...
        for (let i = -1; i <= 1; i++) {
            for (let j = -1; j <= 1; j++) {
                if (i != j && (ig - i) > 0 && (jg - j) > 0) {
                    n = (ig - i) * w + jg - j
                    if (masquedata.data[4 * n] == 255 && !estpresent(germes, ig - i, jg - j)) {

                        canvasdata.data[4 * n] = 255;
                        canvasdata.data[4 * n + 1] = 255;
                        canvasdata.data[4 * n + 2] = 255;
                        germes.push([ig - i, jg - j]);
    ...
    console.log("propagation fin")
    console.log(copygermes)
    console.log(germes);
    if (egaliteTab(germes, copygermes)) {
        return canvasdata;
    } else {
        return propagation(germes, masquedata, canvasdata, w, h);
    }
}
...
function init_textures() {
    ...
    var w = canvas.width;
    var h = canvas.height;
    var masquedata = ctx5.getImageData(0, 0, w, h);
    console.log(masquedata.data)*/
    ...
    var centres = dupliquer(centresTexture11, 10, 10, w1, h1);
